...
4. **Backend** (`backends/`): Abstract `AudioBackend` interface (`base.py`), factory/registry pattern (`factory.py`). Two implementations:
   - **DLNA** (`dlna/`): SOAP/UPnP client (`client.py`), device capability detection (`capabilities.py`), audio proxy server (`proxy_server.py`) — format/quality decisions (`resolve_track`: always ask for the device's true format ceiling, pass through if it fits, else fall back to CD) and plain pass-through streaming. Deliberately generic — no manufacturer knowledge lives here; on-the-fly downsampling is Sonos-specific and layered on via `_resolve_unfitting_stream()`/`_serve()`, two seams `AudioProxyServer` exposes for exactly this rather than a hardcoded branch — see `dlna/sonos/` below.
   - **Sonos** (`dlna/sonos/`): everything Sonos-specific, layered on the generic DLNA classes via subclassing rather than manufacturer-string branching — `SonosClient`/`SonosBackend` (queue-based playback, whole-group volume), `topology.py`/`discovery_manager.py`/`events.py` (continuous household discovery: SSDP + GENA eventing), `controller.py` (`SonosController`, turning discovered rooms into Speaker/Qobuz Connect sessions — see Sonos Auto-Discovery below). On-the-fly Hi-Res downsampling — needed because Sonos's S2 platform is capped at 48kHz regardless of model — lives entirely here too: `proxy_server.py`'s `SonosAudioProxyServer` (subclasses the generic `AudioProxyServer`), `transcode_stream.py` (HTTP serving — HEAD probes, byte-exact-seekable WAV streaming), `transcoding_reader.py` (the FLAC-decode/resample engine, plus `LazyHttpFlacSource`, the file-like bridge that lets libFLAC seek a remote track), and `cdn_block_cache.py` (the block-granular LRU cache/connection-reuse layer every transcoded track's CDN bytes are read through). `BackendFactory` picks `DLNABackend` vs `SonosBackend` by probing the device's manufacturer before connecting; `DLNABackend.create_proxy_server()` is the seam that then picks the matching proxy class (plain vs `SonosAudioProxyServer`) — see `speaker.py`'s proxy construction.
