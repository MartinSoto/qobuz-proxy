...
class DLNACapabilities:
    ...
    def max_quality(self) -> int:
        ...
        (e.g. a 24-bit/48kHz-capped Sonos S2 speaker — see
        apply_device_overrides). Many "Hi-Res" tracks are natively
        mastered at or below such a device's own cap anyway; on the ones
        that aren't, the proxy downsamples on the fly instead of falling
        all the way back to 16-bit (see TranscodingFlacReader) — enforcing
        max_sample_rate is that downstream layer's job, not this one's.
        """
...
def apply_device_overrides(caps: DLNACapabilities, manufacturer: str, model: str) -> None:
    ...
    """
    if "sonos" not in manufacturer.lower():
        return
